...
JOIN khach_hang kh ON kh.ma_khach_hang = hd.ma_khach_hang
WHERE year(hd.ngay_lam_hop_dong)=2021 AND quarter(hd.ngay_lam_hop_dong) BETWEEN 1 AND 2) AND
(year(hd.ngay_lam_hop_dong)=2020 AND quarter(hd.ngay_lam_hop_dong) =4)
GROUP BY hd.ma_hop_dong;

-- 13.Hiển thị thông tin các Dịch vụ đi kèm được sử dụng nhiều nhất bởi các Khách hàng đã đặt phòng.
-- (Lưu ý là có thể có nhiều dịch vụ có số lần sử dụng nhiều như nhau).
CREATE VIEW tong_so_luong AS
...
SELECT ma_dich_vu_di_kem ,ten_dich_vu_di_kem,MAX(so_luong_dich_vu_di_kem) AS max_so_luong
FROM tong_so_luong
WHERE so_luong_dich_vu_di_kem IN (
...
GROUP BY hd.ma_hop_dong,dvdk.ma_dich_vu_di_kem
...
SELECT hd.ma_hop_dong FROM hop_dong hd
JOIN khach_hang kh ON kh.ma_khach_hang = hd.ma_khach_hang
WHERE year(hd.ngay_lam_hop_dong)=2021 AND quarter(hd.ngay_lam_hop_dong) BETWEEN 1 AND 2) 
year(hd.ngay_lam_hop_dong)=2020 AND quarter(hd.ngay_lam_hop_dong) =4
